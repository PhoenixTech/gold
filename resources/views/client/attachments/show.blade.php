            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 p-4 text-center bg-white">
                    ...
                    @if($attachment->subtitle)
                        <p class="text-muted fs-15 mb-2">{{$attachment->subtitle}}</p>
                    @endif
                    @if($attachment->body)
                        <div class="text-muted fs-14 mb-4">{!! $attachment->body !!}</div>
                    @endif
                    <div class="mt-4">
                        @if($attachment->file)
                        ...
                        @else
                            <button class="btn btn-secondary btn-lg rounded-pill px-5 py-2.5 fw-semibold d-inline-flex align-items-center gap-2" disabled>
                                <i class="ri-download-cloud-2-line fs-20"></i>
                                <span>{{__('File not available')}}</span>
                            </button>
                        @endif
                    </div>
                </div>
