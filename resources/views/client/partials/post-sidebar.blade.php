<aside class="post-sidebar">
    <div class="card border-0 shadow-sm rounded-4 p-3 mb-4 bg-white">
        <h5 class="fw-bold fs-15 text-dark mb-3 d-flex align-items-center gap-2">
            <i class="ri-search-2-line text-primary"></i>
            <span>{{__("Search")}}</span>
        </h5>
        <form action="{{route('client.search')}}" method="GET" class="side-data mb-0">
            ...
            @foreach(\App\Models\Post::where('status', 1)->orderByDesc('id')->limit(5)->get() as $pst)
                <li class="mb-3 pb-2 border-bottom last-0">
                    <a href="{{$pst->webUrl()}}" class="d-flex align-items-center gap-2 text-decoration-none text-dark group-hover">
                        <img src="{{$pst->imgUrl()}}" alt="{{$pst->title}}" class="rounded-3 flex-shrink-0" style="width: 52px; height: 52px; object-fit: cover;" loading="lazy">
    ...
    <div class="card border-0 shadow-sm rounded-4 p-3 mb-4 bg-white">
        <h5 class="fw-bold fs-15 text-dark mb-3 d-flex align-items-center gap-2">
            ...
            <span>{{__("Categories / Groups")}}</span>
        </h5>
